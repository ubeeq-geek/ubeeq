...
test('regeneration is scoped, durable, deduplicated and preserves completed outputs until fenced replacement', async () => {
  ...
  try {
    let store = new LocalCreatorLibraryStore(db, { enqueueImageProcessing: true }), queue = new LocalSqliteJobQueue(db);
    ...
    const request = { tenantId: 'tenant', workId: 'work', assetId: 'asset', sourceVersionId: 'v1', expectedRevision: 2, requestId: 'upgrade' };
    ...
    store = new LocalCreatorLibraryStore(db, { enqueueImageProcessing: true }); queue = new LocalSqliteJobQueue(db);
