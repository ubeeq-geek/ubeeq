...
implements CreatorWorkPort<W>, CreatorCollectionPort<C>, CreatorAssetProcessingPort {
  ...
  /** Read-only preflight observation; the mutation constraint rechecks the slug at commit. */
  async hasCreatorContentSlug(tenantId: string, creatorId: string, kind: "work" | "collection", slug: string): Promise<boolean> {
    ...
    const row = this.local.database.prepare(`SELECT NOT (${availableSlug}) AS taken`).get(this.local.configuration.cellId, tenantId, kind, creatorId, "", JSON.stringify([slug])) as { taken: number };
    return row.taken === 1;
  }
