...
const scopeSql = `repository = 'publications' AND json_extract(payload, '$.homeCellId') = ?
  AND json_extract(payload, '$.instanceId') = ? AND json_extract(payload, '$.workId') = ?
  AND json_extract(payload, '$.destination') = ?`;
...
export class LocalWorkPublicationLookup {
  ...
  async list(scope: WorkPublicationScope, request: PageRequest): Promise<Page<PublicationRecord>> {
    ...
    const rows = this.local.database.prepare(`SELECT id, payload FROM ubeeq_records WHERE ${scopeSql} AND id > ? ORDER BY id LIMIT ?`)
