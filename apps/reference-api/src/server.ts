...
const allRecords = async <T>(repository: { list(request: PageRequest): Promise<Page<T>> }): Promise<T[]> => {
  const items: T[] = [];
  for await (const item of repositoryItems(request => repository.list(request))) items.push(item);
  return items;
};
...
export const createReferenceApi = (configuration: ReferenceApiConfiguration): { server: Server; handle(request: IncomingMessage, response: ServerResponse): Promise<void>; runNextJob(workerId: string): Promise<unknown>; close(): Promise<void> } => {
  ...
  const ownedWork = async (workId: string, subjectId: string): Promise<WorkRecord> => {
    ...
    await requireCreatorWritable(creator);
  ...
  const existingImportIds = async () => ({ publication: (await allRecords(repositories.publications)).map(({ id }) => id), publicationIntent: (await allRecords(repositories.publicationIntents)).map(({ id }) => id), moderationEvidence: (await allRecords(repositories.moderationEvidence)).map(({ id }) => id), moderationHold: (await allRecords(repositories.moderationHolds)).map(({ id }) => id), reviewCase: (await allRecords(repositories.reviewCases)).map(({ id }) => id), auditEvent: (await allRecords(repositories.auditEvents)).map(({ id }) => id), usageEvent: (await allRecords(repositories.usageEvents)).map(({ id }) => id), integrationAccount: (await allRecords(repositories.integrationAccounts)).map(({ id }) => id) });
  ...
  const handle = async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      if (method === "POST" && url.pathname === "/v1/works") { const identity = await session(request); const creator = await creatorFor(identity.subject.id); requireHomeCell({ cellId }, creator); await requireCreatorWritable(creator); const body = await parseBody(request); const work = await repositories.works.create({ id: randomUUID(), instanceId: creator.instanceId, ...dataHomeOf(creator), creatorId: creator.id, title: requireString(body.title, "title"), status: "draft" }); return json(response, 201, { work, requestId }, requestId); }
      if (method === "POST" && url.pathname === "/v1/collections") { const identity = await session(request); const creator = await creatorFor(identity.subject.id); requireHomeCell({ cellId }, creator); await requireCreatorWritable(creator); const body = await parseBody(request); const collection = await repositories.collections.create({ id: randomUUID(), instanceId: creator.instanceId, ...dataHomeOf(creator), creatorId: creator.id, title: requireString(body.title, "title"), visibility: body.visibility === "public" || body.visibility === "unlisted" ? body.visibility : "private" }); return json(response, 201, { collection, requestId }, requestId); }

      if (method === "POST" && url.pathname === "/v1/uploads") { const identity = await session(request); const body = await parseBody(request); const work = await ownedWork(requireString(body.workId, "workId"), identity.subject.id); requireHomeCell({ cellId }, work); const byteLength = Number(body.byteLength); if (!Number.isSafeInteger(byteLength) || byteLength < 0) throw new HttpError(400, "invalid_request", "byteLength must be a non-negative integer"); const checksum = typeof body.checksum === "string" && /^[a-f0-9]{64}$/i.test(body.checksum) ? body.checksum.toLowerCase() : undefined; const object = { bucket: cellId, key: cellScopedObjectKey({ cellId, creatorId: work.creatorId, kind: "uploads", objectId: randomUUID() }), contentType: requireString(body.mimeType, "mimeType"), byteLength, checksum, scope: "private" as const }; const upload = await adapters.uploads.initiate({ object, checksumAlgorithm: "sha256", expiresAt: new Date(Date.now() + 15 * 60_000).toISOString() }); return json(response, 201, { upload, requestId }, requestId); }
      const contentMatch = url.pathname.match(/^\/v1\/uploads\/([^/]+)\/content$/);
      if (method === "PUT" && contentMatch) { const identity = await session(request); if (!adapters.uploads.accept) throw new HttpError(405, "direct_upload_required", "Upload content must be sent to the issued direct-upload URL"); const creator = await creatorFor(identity.subject.id); await requireCreatorWritable(creator); const body = await parseBody(request); const base64 = requireString(body.base64, "base64"); await adapters.uploads.accept({ uploadId: contentMatch[1], cellId, creatorId: creator.id, body: Buffer.from(base64, "base64"), operation: "upload_content" }); return json(response, 204, undefined, requestId); }
      ...
      if (method === "GET" && url.pathname === "/v1/exports/me") {
        ...
        const works = (await allRecords(repositories.works)).filter((record) => record.creatorId === creator.id); const workIds = new Set(works.map(({ id }) => id));
        const assets = (await allRecords(repositories.assets)).filter((record) => record.creatorId === creator.id); const collections = (await allRecords(repositories.collections)).filter((record) => record.creatorId === creator.id);
        const publications = (await allRecords(repositories.publications)).filter((record) => workIds.has(record.workId)); const publicationIntents = (await allRecords(repositories.publicationIntents)).filter((record) => workIds.has(record.workId));
        const subjectIds = new Set([creator.id, ...works.map(({ id }) => id), ...assets.map(({ id }) => id)]); const moderationEvidence = (await allRecords(repositories.moderationEvidence)).filter((record) => subjectIds.has(record.subjectId)); const moderationHolds = (await allRecords(repositories.moderationHolds)).filter((record) => subjectIds.has(record.subjectId)); const reviewCases = (await allRecords(repositories.reviewCases)).filter((record) => subjectIds.has(record.subjectId)); const auditEvents = (await allRecords(repositories.auditEvents)).filter((record) => Boolean(record.subjectId && subjectIds.has(record.subjectId))); const usageEvents = (await allRecords(repositories.usageEvents)).filter((record) => record.accountId === creator.id);
        const integrationAccounts = (await allRecords(repositories.integrationAccounts)).filter((record) => record.creatorId === creator.id).map(({ credentialReference: _credentialReference, ...record }) => ({ ...record, credentialExcluded: true as const })); const exportCheckpoints = (await allRecords(repositories.exportManifests)).filter((record) => record.creatorId === creator.id); const importCheckpoints = (await allRecords(repositories.importCheckpoints)).filter((record) => record.creatorId === creator.id);
