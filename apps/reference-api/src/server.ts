...
import { CellRoutingError, composeReferenceApplication, requireHomeCell } from "@ubeeq/api";
...
import { CellOwnershipError, type AssetRecord, type CreatorRecord, type WorkRecord } from "@ubeeq/persistence";
import { cellScopedObjectKey } from "@ubeeq/storage";
...
export interface ReferenceApiConfiguration extends LocalAdapterConfiguration { instanceId?: string; region?: string; operator?: string; federationPolicy?: FederationPolicy; federationVerifier?: FederationSignatureVerifier; mediaProcessor?: MediaProcessor; }

export const createReferenceApi = (configuration: ReferenceApiConfiguration): { server: Server; close(): Promise<void> } => {
  const cellId = configuration.cellId;
  ...
  const handle = async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      if (method === "GET" && publicMatch) { const work = await repositories.works.get(publicMatch[1]); if (!work || work.status !== "published") throw new HttpError(404, "work_not_found", "Published work was not found"); const assets = (await repositories.assets.list({ limit: 100 })).items.filter((asset) => asset.workId === work.id && asset.status === "ready"); const publications = (await repositories.publications.list({ limit: 100 })).items.filter((publication) => publication.workId === work.id && publication.status === "live"); const delivered = await Promise.all(assets.map(async (asset) => { const storage = (asset as AssetRecord & { storage: { key: string; versionId?: string } }).storage; return { ...asset, delivery: await adapters.storage.issue({ object: { bucket: cellId, key: storage.key, versionId: storage.versionId, scope: "public" }, expiresAt: new Date(Date.now() + 3_600_000).toISOString() }) }; })); return json(response, 200, { work, assets: delivered, publications, requestId }, requestId); }
      const deliveryMatch = url.pathname.match(/^\/v1\/delivery\/([^/]+)$/);
      if (method === "GET" && deliveryMatch) { let object; try { object = adapters.storage.verifyDeliveryToken(deliveryMatch[1]); } catch { throw new HttpError(403, "delivery_denied", "Delivery token is invalid or expired"); } if (object.scope !== "public") throw new HttpError(403, "delivery_denied", "This development delivery URL is not public"); const found = await adapters.storage.get(object); response.statusCode = 200; response.setHeader("content-type", found.object.contentType); response.setHeader("content-disposition", object.disposition); response.setHeader("cache-control", "public, max-age=300, s-maxage=3600"); response.setHeader("x-request-id", requestId); response.end(found.body); return; }
...
if (process.argv[1] && process.argv[1].endsWith("server.js")) {
  ...
  const api = createReferenceApi({ databasePath: process.env.UBEEQ_DATABASE_PATH ?? `${dataDirectory}/ubeeq.sqlite`, dataDirectory, publicBaseUrl: process.env.UBEEQ_PUBLIC_BASE_URL ?? `http://127.0.0.1:${port}`, cellId: process.env.UBEEQ_CELL_ID ?? "local-single-cell" });
