...
export class LocalSqliteJobQueue implements JobQueue, Scheduler {
  ...
  async lease<T>(input: { types?: readonly string[]; leaseDurationSeconds: number; workerId: string }): Promise<JobLease<T> | undefined> {
    const row = this.local.database.prepare("SELECT * FROM ubeeq_jobs WHERE state IN ('queued', 'retry_scheduled') AND available_at <= ? ORDER BY available_at, id LIMIT 1").get(now()) as Record<string, unknown> | undefined; if (!row) return undefined;
    const job = this.row(row) as DurableJob<T>; if (input.types?.length && !input.types.includes(job.type)) return undefined;
  ...
  async cancel(input: { id: string }): Promise<void> { this.local.database.prepare("UPDATE ubeeq_jobs SET state = 'cancelled', updated_at = ? WHERE id = ?").run(now(), input.id); }
