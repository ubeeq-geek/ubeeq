...
test("local password identity satisfies the shared development identity contract", async () => {
  ...
  try { await verifyPasswordIdentityContract(createLocalAdapterSet({ databasePath: join(directory, "state.sqlite"), dataDirectory: directory, publicBaseUrl: "http://127.0.0.1" }).identity); }
  finally { rmSync(directory, { recursive: true, force: true }); }
});
