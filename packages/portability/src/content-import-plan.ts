...
export const planCreatorContentImport = (json: string, inventory: CreatorContentImportInventory) => {
  ...
  if (![inventory.targetTenantId, inventory.targetCreatorId].every(validId) ||
    [inventory.existingWorkIds, inventory.existingAssetIds, inventory.existingCollectionIds, inventory.existingSourceFileIds ?? [], inventory.existingPublicationIds ?? [], inventory.existingPublicationIntentIds ?? [], inventory.existingIntegrationAccountIds ?? []].some(values =>
  ...
  for (const [ids, existing] of [[parsed.relatedIds.publications, inventory.existingPublicationIds], [parsed.relatedIds.publicationIntents, inventory.existingPublicationIntentIds], [parsed.relatedIds.integrationAccounts, inventory.existingIntegrationAccountIds]]) if (ids!.length && existing === undefined) throw new Error('Related-record collision inventory is required.');
  ...
  for (const [resource, incoming, existing] of [
    ...
    ['sourceFile', sourceFileIds, inventory.existingSourceFileIds ?? []], ['publication', parsed.relatedIds.publications, inventory.existingPublicationIds ?? []], ['publicationIntent', parsed.relatedIds.publicationIntents, inventory.existingPublicationIntentIds ?? []], ['integrationAccount', parsed.relatedIds.integrationAccounts, inventory.existingIntegrationAccountIds ?? []]
  ] as const) {
