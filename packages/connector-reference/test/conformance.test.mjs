...
test("reference connector keeps OAuth credentials opaque, schedules sync, and exposes normalized health", async () => {
  ...
  await connector.beginOAuth({ id: "oauth-1", integrationId: "reference.connector", ownerId: "creator-1", redirectUri: "https://local.test/callback", requiredScopes: ["works:read"], expiresAt: new Date(Date.now() + 60_000).toISOString() });
  ...
  assert.equal(await connector.enqueueSync({ accountId: "account-1", credentialReference: connected.credentialReference, idempotencyKey: "sync-1" }), "job-1");
