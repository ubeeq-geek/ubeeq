...
export const snapshotCreatorSquareCrop = (value: unknown): CreatorAssetRegenerationRequest['squareCrop'] => {
  ...
  if (Object.keys(crop).some(key => !['x', 'y', 'size'].includes(key)) || ![crop.x, crop.y, crop.size].every(v => typeof v === 'number' && Number.isFinite(v)) || Number(crop.size) <= 0) throw new CreatorAssetRegenerationError('invalid_request', 'Invalid square crop.');
